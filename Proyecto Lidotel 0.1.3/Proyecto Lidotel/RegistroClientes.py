...
def registrar_cliente_acompanado():
    ...
    for i in range(2):
        ...
        while True:
            ...
            # Verificar si la cédula ya está registrada en individuales
            try:
                with open(nombre_del_archivo_individual, "r") as file:
                    registros = file.readlines()
                    for registro in registros:
                        datos = registro.strip().split(",")
                        if len(datos) > 2 and datos[2] == cedula:
                            print("|Error: Ya existe un registro con esta cédula en individuales.")
                            break
                    else:
                        # Si no se encontró la cédula en individuales, verificar en acompañados
                        try:
                            with open(nombre_del_archivo_acompanados, "r") as file:
                                registros = file.readlines()
                                for registro in registros:
                                    datos = registro.strip().split(",")
                                    if len(datos) > 2 and datos[2] == cedula:
                                        print("|Error: Ya existe un registro con esta cédula en acompañados.")
                                        break
                                else:
                                    # Verificar si la cédula ya ha sido registrada en la sesión actual
                                    if cedula in cedulas_registradas:
                                        print("|Error: La cédula ya fue registrada para otro cliente.")
                                    else:
                                        # Si no está registrada, agregar la cédula y continuar
                                        cedulas_registradas.append(cedula)
                                        break
                        except FileNotFoundError:
                            # Si el archivo no existe, proceder normalmente
                            cedulas_registradas.append(cedula)
                            break
            except FileNotFoundError:
                # Si el archivo no existe, proceder normalmente
            ...
            try:
                with open(nombre_del_archivo_individual, "r") as file:
                    ...
                    for registro in registros:
                        ...
                        # Si no se encontró en individuales, verificar en acompañados
                        try:
                            with open(nombre_del_archivo_acompanados, "r") as file:
                                registros = file.readlines()
                                for registro in registros:
                                    ...
                                    # Si no está en ningún archivo, agregar la cédula a la lista
                                    cedulas_registradas.append(cedula)
                                    break
                        except FileNotFoundError:
                            # Si el archivo no existe, proceder normalmente
    ...
    while True:
        ...
        if opcion.isdigit():
            if opcion == "1":
                ...
                input("\n Gracias por usar Lidotel System, hasta la proxima...")
            elif opcion == "2":
                limpiar_consola()
